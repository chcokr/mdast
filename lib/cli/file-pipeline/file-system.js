...
var watchOutputCache = require('../watchOutputCache');
...
function fileSystem(context, done) {
    ...
    var thisFileIsBeingWatched =
        cli.watch && (cli.files.indexOf(file) !== -1);

    if (!thisFileIsBeingWatched) {
        ...
        file.warn('File is being written to, but is also being watched.' +
            ' Thus, instead of being written, the output is being held in' +
            ' memory for you.' +
            ' When you exit the watch with CTRL+C, the latest output will' +
